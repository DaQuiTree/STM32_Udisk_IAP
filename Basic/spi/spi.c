...
void SPI2_Init(void){ //SPI2初始化
	...
	GPIO_InitStructure.GPIO_Pin = SPI2_MOSI | SPI2_SCK;	//SPI2的MO  SI（PB15）和SCLK（PB13）为复用推免输出
...
void SPI2_NSS_Enable(void)
{
  GPIO_WriteBit(SPI2PORT, SPI2_NSS, Bit_RESET);
}

void SPI2_NSS_Disable(void)
{
  GPIO_WriteBit(SPI2PORT, SPI2_NSS, Bit_SET);
}
/*********************************************************************************************
